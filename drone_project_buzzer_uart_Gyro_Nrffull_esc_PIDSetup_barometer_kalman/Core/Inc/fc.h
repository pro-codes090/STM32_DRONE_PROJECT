...
#define _esc_desetup_(mode)  	LL_TIM_DisableCounter(TIM1);\
								TIM1->BDTR &= ~(1 << 15);\
...
#define BUZZER_CALIB_MODE(mode)	LL_TIM_EnableCounter(TIM3);\
								LL_TIM_CC_EnableChannel(TIM3, LL_TIM_CHANNEL_CH1);\
								HAL_Delay(100) ;\
...
// TIM3 psc is 999 and ahb1 freq is 42mhz
#define BUZZER_ARMED(mode)	LL_TIM_EnableCounter(TIM3);\
								LL_TIM_CC_EnableChannel(TIM3, LL_TIM_CHANNEL_CH1);\
								HAL_Delay(100) ;\
								LL_TIM_SetAutoReload(TIM3, 84) ;\
								HAL_Delay(100) ;\
								LL_TIM_SetAutoReload(TIM3, 70) ;\
								HAL_Delay(100) ;\
								LL_TIM_SetAutoReload(TIM3, 60) ;\
								HAL_Delay(100) ;\
								LL_TIM_SetAutoReload(TIM3, 52) ;\
								LL_TIM_CC_DisableChannel(TIM3, LL_TIM_CHANNEL_CH1) ;

#define BUZZER_UNARMED(mode)	LL_TIM_EnableCounter(TIM3);\
								LL_TIM_CC_EnableChannel(TIM3, LL_TIM_CHANNEL_CH1);\
								LL_TIM_SetAutoReload(TIM3, 56) ;\
								HAL_Delay(100) ;\
								LL_TIM_SetAutoReload(TIM3, 70) ;\
								HAL_Delay(150) ;\
								LL_TIM_SetAutoReload(TIM3, 93) ;\
								HAL_Delay(200) ;\
								LL_TIM_SetAutoReload(TIM3, 120) ;\
								HAL_Delay(250) ;\
								LL_TIM_CC_DisableChannel(TIM3, LL_TIM_CHANNEL_CH1) ;
