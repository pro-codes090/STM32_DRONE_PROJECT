...
static void cs_high()
{
    LL_GPIO_SetOutputPin(NRF24L01P_SPI_CS_PIN_PORT , NRF24L01P_SPI_CS_PIN_NUMBER); // set
}

static void cs_low()
{
	LL_GPIO_ResetOutputPin(NRF24L01P_SPI_CS_PIN_PORT , NRF24L01P_SPI_CS_PIN_NUMBER); // reset
}

static void ce_high()
{
    LL_GPIO_SetOutputPin(NRF24L01P_CE_PIN_PORT , NRF24L01P_CE_PIN_NUMBER) ; // set
}

static void ce_low()
{
	LL_GPIO_ResetOutputPin(NRF24L01P_CE_PIN_PORT, NRF24L01P_CE_PIN_NUMBER) ; // reset
}
...
//    HAL_SPI_Receive(NRF24L01P_SPI, &read_val, 1, 2000);
	...
	while((! (LL_SPI_IsActiveFlag_TXE(SPI2))) || ( LL_SPI_IsActiveFlag_BSY(SPI2)) || (LL_SPI_IsActiveFlag_RXNE(SPI2)) ) {}
	LL_SPI_TransmitData8(SPI2, command ) ;
    while((! (LL_SPI_IsActiveFlag_TXE(SPI2))) || ( LL_SPI_IsActiveFlag_BSY(SPI2)) || (!(LL_SPI_IsActiveFlag_RXNE(SPI2))) ) {}
	status = LL_SPI_ReceiveData8(SPI2) ; // dummy read
	while((! (LL_SPI_IsActiveFlag_TXE(SPI2))) || ( LL_SPI_IsActiveFlag_BSY(SPI2)) ||  (LL_SPI_IsActiveFlag_RXNE(SPI2)) ) {}
	LL_SPI_TransmitData8(SPI2, 0xff) ;
	while((! (LL_SPI_IsActiveFlag_TXE(SPI2))) || ( LL_SPI_IsActiveFlag_BSY(SPI2)) || (!(LL_SPI_IsActiveFlag_RXNE(SPI2)))) {}
	read_val = LL_SPI_ReceiveData8(SPI2) ;
...
//    HAL_SPI_Transmit(NRF24L01P_SPI, &write_val, 1, 2000);
	...
	status = LL_SPI_ReceiveData8(SPI2) ; // dummy read
	while((! (LL_SPI_IsActiveFlag_TXE(SPI2))) || ( LL_SPI_IsActiveFlag_BSY(SPI2)) || (LL_SPI_IsActiveFlag_RXNE(SPI2)) ) {}
	LL_SPI_TransmitData8(SPI2, write_val) ;
    while((! (LL_SPI_IsActiveFlag_TXE(SPI2))) || ( LL_SPI_IsActiveFlag_BSY(SPI2)) || (!(LL_SPI_IsActiveFlag_RXNE(SPI2))) ) {}
